...
| `tsc --strict` (typecheck)                                                                         | `pnpm typecheck`      | Erros de tipo em geral                                                                                                                                                                                                  | **Atualizado na F1-E1:** `noUncheckedIndexedAccess` passou a ficar **ligado em `src/sim/`** (P-02 fechada, `src/sim/tsconfig.json`), e continua **desligado** em `src/render/`, `src/app/` e `src/platform/` — lá o acesso indexado (`array[i]`) não é forçado a `T \| undefined`. Medido ao ligar: o núcleo não precisou de nenhuma correção (o `pick()` de `rng.ts` já tinha a checagem explícita), e todo o `src/render/` + `src/app/` também passa com a flag ligada, então a isenção do renderer é hoje mais folgada do que o necessário |
